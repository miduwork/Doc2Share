...
export default async function AdminDocumentsPage({ searchParams }: { searchParams?: SearchParams }) {
  ...
  const status = pickSingle(searchParams?.status, preset === "failed" ? "failed" : preset === "deleted" ? "deleted" : "all");
  ...
  return (
    <div>
      <AdminPageHeader
        title="Tài liệu"
        description="Đăng, duyệt và quản lý tài liệu — pipeline xử lý bên dưới"
      />
      <div
