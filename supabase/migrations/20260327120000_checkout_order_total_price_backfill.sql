-- Đơn checkout: luôn ghi total_price = số tiền VND (int) để webhook đối soát ổn định.
-- Trước đây INSERT không set total_price → NULL; client không UPDATE được do RLS.
...
BEGIN
  ...
  INSERT INTO orders (
    id, user_id, total_amount, total_price, status, external_id, order_items, created_at, updated_at
  )
  VALUES (
    v_order_id,
    v_user_id,
...
SET total_price = floor(total_amount)::int4
